...
struct App {
    ...
    should_quit: bool,
    /// Help information for user commands
...
impl App {
    ...
    fn new() -> Self {
        ...
        Self {
            ...
            help_info: vec![
                ...
                "  Enter: View details".to_string(),
                "".to_string(),
                "│Commands:".to_string(),
                "  q: Quit".to_string(),
                "  h: Toggle help".to_string(),
                "  c: Clear messages".to_string(),
                "  f: Filter messages (TODO)".to_string(),
                "".to_string(),
                "│Status:".to_string(),
                "  Port: 8765".to_string(),
    ...
    fn format_message(&self, record: &MessageRecord) -> String {
        ...
        let message_type = match record.message.header.message_type {
            MessageType::Handshake => "Handshake / 握手",
            MessageType::Data => "Data / 数据",
            MessageType::Control => "Control / 控制",
            MessageType::Heartbeat => "Heartbeat / 心跳",
            MessageType::Error => "Error / 错误",
        };
        format!(
            "{} {} [{}] Session ID: {} / 会话ID: {}",
            direction,
            timestamp,
            message_type,
            record.message.header.session_id,
            record.message.header.session_id
        )
    }
        ...
        terminal.draw(|f| {
            ...
            let chunks = Layout::default()
                ...
                .constraints(
                    [
                        ...
                        Constraint::Length(if show_help { 4 } else { 0 }), // Help info
                    ]
                    .as_ref(),
                )
                .split(f.size());
            ...
            let details = if let Some(selected) = app.list_state.selected() {
                if let Some(record) = app.messages.get(selected) {
                    Paragraph::new(app.format_message_details(record)).block(
                        Block::default()
                            .borders(Borders::ALL)
                            .title("Message Details / 消息详情"),
                    )
                } else {
                    Paragraph::new("No message selected / 未选择消息").block(
                        Block::default()
                            .borders(Borders::ALL)
                            .title("Message Details / 消息详情"),
                    )
            ...
            if show_help {
                let help_text = app.help_info.join("\n");
            ...
            match key {
                ...
                event::KeyCode::Enter => {
                    if let Some(selected) = app.list_state.selected() {
                        if let Some(record) = app.messages.get(selected) {
                            println!("{}", app.format_message_details(record));
                        }
                    }
                }
                event::KeyCode::Up => {
